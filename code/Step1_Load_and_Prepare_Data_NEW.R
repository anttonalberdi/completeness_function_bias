...
## BIN quality
quality <- read.delim(gzfile("data/mag_data/DRAM/genome_stats.tsv.gz"),sep="\t")
...
# Remove first column
taxmatrix <- taxonomyclean[,c(11:17)]
# Set MAG name as row names of the dataframe
rownames(taxmatrix) <- taxonomyclean[,1]
...
# Keep functions present in at least 10 MAGs in dram table
dram_product_ColRed <- dram_product[,colSums(dram_product>0)>296]
# Keep functions present in at least 10 MAGs in n_steps table
n_steps_RowRed <- n_steps[colSums(dram_product>0)>296,]
# Keep functions present in at least 10 MAGs in hierarchy table
module_hierarchy_RowRed <- module_hierarchy[colSums(dram_product>0)>296,]
